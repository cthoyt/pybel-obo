...
from pybel_obo import get_relationship
...
class TestConversion(unittest.TestCase):
    ...
    def get_relationship(self, source: BaseEntity, target: BaseEntity, key: str) -> Mapping[str, str]:
        """Wrap conversion on this test case's graph."""
        return get_relationship(self.graph, source, target, key)
    ...
    def test_regulates_transport(self):
        ...
        key = self.graph.add_increases(protein_1, protein_2, evidence=n(), citation=n(), object_modifier=secretion())
        self.assertEqual(r, self.get_relationship(protein_1, protein_2, key))

        key = self.graph.add_directly_increases(protein_1, protein_2, evidence=n(), citation=n(),
                                                object_modifier=secretion())
        self.assertEqual(r, self.get_relationship(protein_1, protein_2, key))

        key = self.graph.add_decreases(protein_1, protein_2, evidence=n(), citation=n(), object_modifier=secretion())
        self.assertEqual(r, self.get_relationship(protein_1, protein_2, key))

        key = self.graph.add_directly_decreases(protein_1, protein_2, evidence=n(), citation=n(),
                                                object_modifier=secretion())
        self.assertEqual(r, self.get_relationship(protein_1, protein_2, key))

        key = self.graph.add_qualified_edge(protein_1, protein_2, REGULATES, evidence=n(), citation=n(),
                                            object_modifier=secretion())
        self.assertEqual(r, self.get_relationship(protein_1, protein_2, key))
    ...
    def test_represses_expression_of(self):
        ...
        key = self.graph.add_directly_decreases(protein_1, rna_2, evidence=n(), citation=n(),
                                                subject_modifier=activity())
        self.assertEqual(r, self.get_relationship(protein_1, rna_2, key))
    ...
    def test_increases_expression_of(self):
        ...
        key = self.graph.add_directly_increases(protein_1, rna_2, evidence=n(), citation=n(),
                                                subject_modifier=activity())
        self.assertEqual(r, self.get_relationship(protein_1, rna_2, key))
    ...
    def test_is_substance_that_treats(self):
        ...
        key = self.graph.add_decreases(abundance_1, pathology_1, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(abundance_1, pathology_1, key))
    ...
    def test_causes_condition(self):
        ...
        key = self.graph.add_increases(abundance_1, pathology_1, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(abundance_1, pathology_1, key))

        key = self.graph.add_increases(gene_2, pathology_1, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(gene_2, pathology_1, key))

        key = self.graph.add_directly_increases(gene_2, pathology_1, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(gene_2, pathology_1, key))
    ...
    def test_over_expressed_in(self):
        ...
        key = self.graph.add_qualified_edge(protein_1, pathology_1, POSITIVE_CORRELATION, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, pathology_1, key))
    ...
    def test_under_expressed_in(self):
        ...
        key = self.graph.add_qualified_edge(protein_1, pathology_1, NEGATIVE_CORRELATION, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, pathology_1, key))
    ...
    def test_transcribed_from(self):
        ...
        self.assertEqual(r, self.get_relationship(rna_1, protein_1, key))
    ...
    def test_ribosomally_translates_to(self):
        ...
        self.assertEqual(r, self.get_relationship(gene_1, rna_1, key))
    ...
    def test_phosphorylates(self):
        ...
        key = self.graph.add_increases(protein_1, v_phosphorylated, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_phosphorylated, key))

        key = self.graph.add_directly_increases(protein_1, v_phosphorylated, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_phosphorylated, key))

        # Check mechanistic BioPAX-style relations
        key = self.graph.add_increases(protein_1, v_phosphorylation_reaction, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_phosphorylation_reaction, key))

        key = self.graph.add_directly_increases(protein_1, v_phosphorylation_reaction, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_phosphorylation_reaction, key))
    ...
    def test_ubiquitinates(self):
        ...
        # Check simple BEL-style relations
        key = self.graph.add_increases(protein_1, v_ubiquitinated, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_ubiquitinated, key))

        key = self.graph.add_directly_increases(protein_1, v_ubiquitinated, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_ubiquitinated, key))

        # Check mechanistic BioPAX-style relations
        key = self.graph.add_increases(protein_1, v_ubiquitination_reaction, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_ubiquitination_reaction, key))

        key = self.graph.add_directly_increases(protein_1, v_ubiquitination_reaction, evidence=n(), citation=n())
        self.assertEqual(r, self.get_relationship(protein_1, v_ubiquitination_reaction, key))
